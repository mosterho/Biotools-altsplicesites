...
class class_altsplice:
    ...
    def __init__(self, arg_gene, arg_print=''):
        self.input_gene, self.input_print = arg_gene, arg_print

    def get_altsplice(self):
        ...
        gcursor = collect.aggregate([ {"$match":{"gene_id":{"$eq": self.input_gene}}}, {"$group": {"_id": {"gene_id":"$gene_id"}, "total_g":{"$sum":1}}} ])
        for g_row in gcursor:
            wrk_mrna_count = g_row['total_g']
            if(self.input_print == 'Y'):
        ...
        if(self.input_print == 'Y'):
            print('Number of mRNA associated with gene ',self.input_gene,' is: ',wrk_mrna_count)
        ...
        mcursor1 = collect.aggregate([ {"$match":{"gene_id":{"$eq": self.input_gene}}}, {"$unwind":"$exons"}, {"$group": {"_id": {"gene_id":"$gene_id", "exonstart":"$exons.start", "exonend":"$exons.end"}, "total":{"$sum":1}}}, {"$sort":{"exonstart":1,"exonend":1}}])
        ...
        mcursor2 = collect.aggregate([{'$match':{'gene_id':{'$eq':self.input_gene}}} , {'$unwind':"$exons"}, {'$sort':{ "exons.start":1, "exons.end":1, 'accession':1}}, {'$project':{'_id':0,  'gene_id':1, 'accession':1, 'exons':1 }}])
        ...
        return self.wrk_mrnafromtoexon
